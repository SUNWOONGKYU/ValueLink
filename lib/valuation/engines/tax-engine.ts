...
class TaxEngine extends ValuationEngine {
   ...
   * 2. 총자산(assets) > 0
   * 3. 총부채(liabilities) >= 0
   * 4. 수익(earnings) - 음수 허용 (적자 기업)
   * 5. 할인율(discountRate) 0 < x < 1
   *
   * [Phase 4 수정] 손실 기업 허용:
   * - earnings <= 0일 경우 수익가치 = 0으로 계산
   * - NAV 100% 가중치로 평가 진행
   *
  ...
  validate(data: ValuationInput): ValidationResult {
    ...
    // [Phase 4 수정] earnings 음수 허용
    if (data.earnings === undefined || data.earnings === null) {
      return {
        valid: false,
        error: '순이익(earnings) 값이 제공되어야 합니다. (음수 가능)',
      };
    }
   ...
   *   1. 순자산가치(NAV) = 자산 - 부채
   *   2. 수익가치 = 순이익 / 할인율 (환원법)
   *      - [Phase 4 수정] earnings <= 0이면 수익가치 = 0
   *   3. 가중평균 = NAV * navWeight + 수익가치 * (1 - navWeight)
   *      - [Phase 4 수정] earnings <= 0이면 NAV 100% 사용
   *   4. 주당가치 = 가중평균 / 발행주식수
   *
   * @param data - 평가 입력 데이터
  ...
  async calculate(data: ValuationInput): Promise<ValuationResult> {
    ...
    try {
      ...
      // ----- 2. 수익가치 (환원법) -----
      //   [Phase 4 수정] 손실 기업 (earnings <= 0) 처리
      let earningsValue = 0;
      let effectiveNavWeight = data.navWeight || 0.6;
      let effectiveEarningsWeight = 1 - effectiveNavWeight;
      let lossCompanyWarning: string | undefined;

      if (data.earnings! <= 0) {
        // 손실 기업: 수익가치 = 0, NAV 100% 사용
        earningsValue = 0;
      ...
      //   적자 기업: 순자산 100%
      const weightedValue =
        netAssetValue * effectiveNavWeight + earningsValue * effectiveEarningsWeight;
      ...
      // 손실 기업 경고 추가
      if (lossCompanyWarning) {
        result.details.lossCompanyWarning = lossCompanyWarning;
      }
