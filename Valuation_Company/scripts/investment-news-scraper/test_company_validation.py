...
# ──────────────────────────────────────────────────────────
MUST_REJECT = [
    # 06-29 라이브 노출 노이즈 (deal.html 실측 발견)
    ('이번엔 유니콘브릿지 선정', '기사 동사구 조각(선정)'),
    ('신한·키움',         '금융사 나열 조각(투자자 오추출)'),
    # 06-28 라이브 노출 노이즈 (스케줄러 06-28 08:00분 통과분)
    ('로봇 스타트업에만', '복합 보조사 에만으로 끝나는 문장조각'),
    ('복부암',          '질병명(회사명 아님)'),
    ('뉴욕 메츠',        '스포츠 구단명(회사명 아님)'),
    ('한국 기공사 실력',  '기사 명사구 조각(실력=일반어)'),
    # 2글자 명백 일반어
    ('사줘',            '2글자 동사어미'),
    ('추론',            '2글자 일반명사'),
    ('창업',            '2글자 일반명사'),
    ...
    ('미국은 빅테크',         '조사 은 + NOISE'),
    # 2026-06-26 노이즈 배치 — 대학/대기업/투자사/언론/기사조각/인명
    ('호서대',                '대학 약칭'),
    ('호서대학교',            '대학'),
    ('호서대 산학협력단',     '산학협력단'),
    ('삼성전자',              '대기업'),
    ...
    ('자율주행 확장 가속',    '기사조각 가속'),
    ('혁신성·성장성 인정',    '기사조각 인정'),
    ('기술성·성장 잠재력 인정', '잠재력/인정'),
    ('다양한 중추신경계 치료제', '치료제'),
    ('중소게임업체',          '업체'),
    ('이상훈 에이비엘바이오 대표', '인명+대표'),
    ('세일 리서치',           '리서치'),
    ('프로젝트 제타',         '프로젝트'),
    ('IMM인베',               'VC약칭 인베(6자미만)'),
    ('최기영 봉화군수',       '인명+군수'),
...
REGRESSION = [
    ...
    ('셀트리온',            '실딜'),
    ('앤스로픽',            '실딜 — NOISE에서 제거됨'),
    # 2026-06-26 배치의 실제 회사 — 노이즈 강화 후에도 통과 필수
    ('오소고널',            '실딜'),
    ('이노바이드',          '실딜'),
    ('모먼트 에너지',       '실딜 — 공백1'),
    ('에즈큐리스',          '실딜'),
    ...
    ('코트라스',            '실딜'),
    ('런팟',                '실딜'),
    # 2026-06-27 넓은스캔의 실제 회사 — 강화 후에도 통과 필수
    ('팀리미티드',          '실딜'),
    ('센티넬딥액티브',      '실딜'),
    ('에어월렉스',          '실딜 Airwallex'),
    ('백경증류소',          '실딜 증류소'),
...
REASON_CASES = [
    ...
    ("양산 체제 구축 위해 200억 유치",         '양산 체제',  "구체 구절 우선 ('양산 체제')"),
    # 확장(2026-06-30 PO "몇 자라도 포함") — 폭넓게 추출
    ("북미 진출과 인프라 구축에 나선다",        '북미 진출',  '확장 키워드'),
    ("물류 효율화에 투입하기로 했다",          '물류 효율화 위해', '투입 패턴 — 몇 자라도'),
    ("운영자금 마련을 위해 30억 유치",         None,        "'자금/마련'은 약한 사유 → 배제"),
]
