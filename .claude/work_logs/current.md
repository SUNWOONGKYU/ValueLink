...
## 2026-07-16 평가 에이전트 2종 제조 + 제품 상증세법·DCF 엔진 결함 교정 ✅완료

### 작업 상태: ✅ 완료 (3계층 작업팀, V①·V② 이중 검증 전 항목 통과)

### 산출물 1 — 로컬 설치형 에이전트 2종 (바탕화면, 자산 미등재)
- `Desktop\dcf-valuation-agent\` — DCF 기업가치평가 에이전트
- `Desktop\tax-valuation-agent\` — 상증세법 비상장주식 평가 에이전트
- 구성: saa.house 가이드 기준 기본 5요소 + 강화 2종(system_prompt 페르소나, knowledge 간이 RAG) + 안전 체계(방어 5: 입력 가드레일·출력 정합성 가드+AI 생성물 표시·실행 제한·HITL·재시도 폴백 / 점검 3: trace 로그·비용 계측·입력 부록)
- 각 에이전트 docs\에 관계도.html·흐름도.html·설명문.md (V① 코드 대조 + 소대장 렌더링 검증 통과)
- 기존 제품 엔진 재활용, LLM 두뇌는 claude CLI(구독) + 규칙 기반 폴백

### 산출물 2 — 제품 저장소 엔진 결함 교정 (이번 커밋)
- `tax/tax_law_engine.py` 전면 교정: 구 엔진이 순손익가치를 평균×3÷10%(=합계÷10%)로 **약 3배 과대계상**, 법적 근거 없는 소액주주·유동성 할인 적용, 80% 하한 누락 → 시행령 제54조·56조, 법 제63조③ 현행 조문(법제처 공식 API 원문 대조, 2026-02-27 시행)대로 재작성. 가중평균(3:2:1)÷6→÷환원율(파라미터 10%), 음수→0, 3:2/2:3, 순자산가치 80% 하한, 최대주주 20% 할증(배제: 중소·중견·3년 연속 결손 자동판정), 임의 할인 제거. 6케이스 수기 검산 3중(분대장·V①·소대장) 일치.
- `dcf/sensitivity_analysis.py`: 민감도 매트릭스·시나리오의 ×1,000,000 단위 변환 누락(전 셀 "0원" 버그) 수정.
- `dcf/dcf_engine.py`: __main__·docstring 단위 함정(원 단위 예시 vs 백만원 공식 → 주당가치 100만 배 왜곡) 백만원 규약으로 정렬.
- `valuation_orchestrator.py`: 신 엔진 시그니처로 호출부 갱신 (API 미연결 고아 코드 확인됨, 호환 래퍼 대신 직접 갱신 — 부정확 근사 재도입 방지).
- 참고: 라이브 사이트 계산기(frontend tax-valuation.html)는 원래부터 법령대로 정확 — 수정 불요 확인.

### 검증
- 편제: 소대장(Opus)–분대장A/B/C(Sonnet)–V①(Sonnet)→V②(Opus) 이중 게이트, 반려→수정→재검증 순환 다수 회전
- 실행 검증: 제품 엔진 단독 실행 6케이스 assert 통과(EXIT 0, cp949 콘솔 포함), DCF 226,663/211,663/21,166백만원·원 재현
- 미결: backend는 미배포 상태. orchestrator에 실제 API가 원/백만원 어느 단위로 보내는지는 향후 backend 활성화 시 확인 필요

### 부수 인프라
- korean-law MCP 설치 (OC=wksun999, `~/.claude.json` user 스코프) — 법령 검증 표준 경로화

### 추가 기능 (지휘관 지시, 같은 날 후속)
- 에이전트 2종에 **HTML 보고서 + PDF 저장** 기능 추가 — report_html.py 신설(계산 로직 불변), 매 실행 시 .md·.json과 함께 자체완결 .html 저장(인라인 CSS/JS만, XSS 이스케이프), 우상단 "PDF로 저장" 버튼(window.print, @media print A4 정리)
- 검증: V①(회귀·이스케이프·HTML-MD 일치·외부리소스 0건) 통과 + 소대장 라이브 테스트(실행→렌더 스크린샷→실제 PDF 생성 125~157KB→버튼 클릭 스파이 호출 확인). 반려 1건(영업이익률 소수 자릿수 불일치 .2%→.1%) 수정 재검증 통과

---

## 2026-07-01 딜뉴스 투자이유 AI 상세 추출 (PO 지시: 상세하게·요약금지) ✅완료

### 작업 상태: ✅ AI 상세 추출 + null 판정/도출 + 정리

### 배경
- PO 보정: "몇 자라도 포함"은 포함기준이지 짧게 요약하란 게 아님. 기사에 상세히 나오면 상세하게 기재
- 규칙기반 키워드 추출은 요약체라 부적합 → PO 승인: AI(claude CLI)로 상세 추출

### 수행
- 로컬 claude CLI(`claude -p`)로 각 기사 본문 읽어 자금 사용목적을 상세 서술로 추출 (배치8, 백그라운드)
  - 299건 → 상세사유 178건(평균 55자) / null 121건
- null 121건 재판정(AI): 국내 실투자딜이면 회사 사업내용 기반 짧은 사유 도출, 아니면 REMOVE
  - 유지 45건(사유 도출) / 삭제 76건(정책·수상·지원선정·해외·투자자측·조각 등 노이즈)
- 삭제 백업: inbox/deals_noreason_ai_backup_20260701.json (76)

### 최종 deals
- **231건 / 투자이유 100% 기재 / 평균 45자 상세 / 불량(매체명·과길이) 0**
- 예: 알체라 "피지컬AI 데이터 파이프라인 구축에 투입, 휴머노이드·제조 등 고부가 데이터 시장 확장 위해" 등 서술형

### ⚠️ 미결 (PO 확인 필요)
- **일일 자동수집(collect_naver_only.py)은 여전히 규칙기반 키워드 추출**. AI 상세추출을 매일 무인 배치에 넣을지는 별도 결정(무인 claude CLI 안정성·비용). 현재 신규 수집분은 키워드 사유

---

## 2026-06-30 투자이유 추출 포용성 강화 ("몇 자라도 포함") + 삭제분 복원 (PO 보정) ✅완료

### 작업 상태: ✅ 추출기 개선 + 복원 + 품질 재정리

### 배경
- PO 보정: "투자이유 몇 자라도 적을 수 있는 건 포함하라" → 기존 보수적 추출이 과도하게 미기재 처리한 것 완화

### 수행
- `extract_reason` 확장: 목적 키워드 대폭 추가(진출/기술/생산/상장/출시 등) + "○○ 위해/목적으로/투입/활용" 패턴
- 부작용 교정(반복): 사이트명('TokenPost 위해')·문장조각(28자) 혼입 → 한글필수 + 매체명 배제 + **짧은 구절(2~12자)·조사로 시작 금지**로 엄격화
- 삭제했던 미기재 170건 재처리(개선 추출+국내필터) → 105건 복원 후, '위해' 사유 2회 재정리(불량 33건 삭제)
- 단위테스트 REASON 10케이스로 확장(확장·배제 사례 포함), 전체 ALL PASS

### 최종 deals
- **299건 / 투자이유 100% 기재 / 전부 국내 / 해외 0 / 불량사유(>15자·매체명) 0**
- 사유 분포: 마케팅31·연구개발30·시장진출26·고도화13·글로벌진출12 … 전부 짧고 깔끔
- 추이: 227(직전) → 복원·정리 → 299 (순증 72)

---

## 2026-06-30 딜뉴스 국내 기업 한정 + 등재 게이트 규칙화 (PO 지시) ✅완료

### 작업 상태: ✅ 기존 정리 + 규칙·스크래퍼·테스트 반영

### PO 방침 (2건, 규칙화)
1. **국내 기업 한정** — 해외 기업이 투자받은 건 제외(국내사 해외진출·해외투자자 유치는 유지)
2. **투자이유 필수** — 못 채우면 목록에 등재 안 함

### 수행
- 기존 233건에서 해외/비투자 6건 백업 후 삭제(어피닛=인도/세리액트=독일/가이아=미국/크루캐피탈조각/레이븐=선정/셀트리온=투자아님) → 227
- `collect_naver_only.py`: `_is_foreign_company(title, company)` 추가(회사명 직전 '○○국 …기업/스타트업'만 해외 판정 — 해외 투자자·해외확장은 국내 유지) + 수집부에 해외 스킵·투자이유 None 스킵
- `test_company_validation.py`: FOREIGN 8케이스 추가 → 전체 ALL PASS (FOREIGN 8/8 포함)
- `.claude/rules/08_article-selection.md`: §0 등재 자격 게이트(국내 한정 + 투자이유 필수) 신설
- 현재 227건 전체에 정교화 필터 재적용 → 해외 잔존 0 확인

### 최종 deals 상태
- 227건, 전부 국내 기업 + 투자이유 100% 기재
- 백업: inbox/deals_foreign_backup_20260630.json (6)
- ⚠️ 향후 영향: 신규 수집 시 투자이유 없는 딜(최근 ~73%)·해외 기업 자동 제외 → 등재량 감소하나 PO 방침대로 품질 우선

---

## 2026-06-30 딜뉴스 노이즈 정리 + 투자이유 백필·품질정제 (PO 지시 2→1, "모두 정리") ✅완료

### 작업 상태: ✅ DB 정리 완료 + 백업·검증

### 배경
- "딜뉴스에 등록된 것 전부 투자이유 기재됐나?" 점검 → 423건 중 277기재(겉보기 65%)였으나 상당수가 쓰레기
- PO: 2→1(노이즈 정리 후 백필) → 이후 "모두 다 정리"(중복·품질까지)

### 수행 (PAT Management API + 스크래퍼 로직 재사용)
1. 노이즈 식별: `_valid_company` 재사용 → 423중 26 검증실패. 판정: 16삭제/7명정정/3유지(오탐 메카AI·스테이터스AI·제네시스AI)
2. 노이즈 16건 백업 후 삭제(423→407), 7건 회사명 정정(공간지능OS→비저너리 등)
3. 투자이유 백필: 미기재 138건 news_url 재수집+extract_reason → 75 추출
4. 이름정정 부작용 중복 4건(비저너리·바이트랩·아크·에임인텔리전스) 백업 후 삭제(407→403)
5. 품질 정제: 기존 "기재"의 257건이 구버전 쓰레기(원문문장·[푸터]·태그)였음 → 백업 후 재추출(150 깔끔/107 비움)

### 최종 결과
- deals 403건 / 깔끔한 투자이유 233건(58%) / 미기재 170건
- 쓰레기 마커(길이>33·괄호·문장어미) 잔존 0 검증. 사유 전부 PURPOSE_KEYWORD 또는 "○○ 위해"
- ⚠️ 종전 "85%"는 쓰레기 포함 허수 — 실제 깔끔 기재율 58%가 정직한 값
- 미기재 170건은 기사에 명확한 목적 서술이 없어 보수적으로 비움(추측 안 함)

### 백업 (전부 복구 가능, inbox/)
- deals_noise_backup_20260630.json(16) / deals_dup_backup_20260630.json(4) / deals_reason_qfix_backup_20260630.json(257 원본)
- 검증 스크립트 등 임시파일 전량 삭제

### 추가 (PO 방침: "투자이유 못 채우면 목록에서 빼라")
- 미기재 170건 백업 후 삭제 → deals 403→233. **233건 전부 투자이유 100% 기재** (미기재 0)
- 백업: inbox/deals_noreason_backup_20260630.json (170)
- ⚠️ 미결: 향후 수집(collect_naver_only.py)도 투자이유 없는 딜은 저장 안 하도록 할지 PO 확인 대기
  (최근 수집분 기재율 ~27%라, 적용 시 신규 딜 다수가 미수집됨 — 영향 큼)
...
### 배경
- 직전 세션 핸드오프 손상(구독 비활성화 메시지로 덮임). 직전 배포 3건(결제예금주/상대가치버그/샘플정리)은 이미 커밋·배포 완료 확인. PO "1·2·3 다 진행".

### Phase 1: 안정화 라이브 검증 (발견 버그 0)
- 5개 평가법 계산: 라이브 SIM_BASE 32/32 PASS / 보고서 생성: 라이브 5/5 PASS
- 가입(200+메일발송)/로그인(미확인계정 거부=의도된 동작) 정상
- 마이페이지: 하드코딩 더미 0건, 세션훅 실데이터 연결됨 (06-26 보류 항목 해결 확인)
- 직전 배포 3건 라이브 재검증: 결제 예금주'선명회계법인'·하나은행/28791092140507, 상대가치 inputs[6] 수정본, 샘플테크 교체 — 전부 반영 확인
- 삼성전자 5파일은 정당한 placeholder/교육예시 (제거 대상 아님)

### Phase 2: 미커밋 잔여물 정리 (비파괴적)
- untracked 138개 분류 → 생성물/임시/구버전 스크래퍼만 .gitignore 처리 (파일 삭제 0)
- untracked 138→98. worklog/SQL/migration/하니스/문서는 보존
- 커밋 75ff9e8

### Phase 3: 딜뉴스 자동수집 점검 + 노이즈 필터 강화
- 스케줄러 ValueLink-DealNews 정상 (06-28 08:00 실행, 결과 0, 6건→425건)
- deal.html 최신 딜 정상 표시. deal-live-check 0/14는 기대목록 노후화(버그 아님)
- **노이즈 4건 라이브 노출 발견** (복부암/뉴욕 메츠/로봇 스타트업에만/한국 기공사 실력)
- 필터 패치: _valid_company 복합보조사 어미('에만'등) + NON_COMPANY_KEYWORDS '실력' + EXTRA_NOISE2 질병/스포츠 안전망
...
- Valuation_Company/scripts/investment-news-scraper/test_company_validation.py (테스트 4건)
- .gitignore (잔여물 정리), _WorkLog/2026_06_28__21.40_PHASE_*.md
...
### 배경/방침
- PO 방침: 신규기능 개발 중단, 기존 1차 평가서비스 완벽동작에만 집중. 결제는 무통장 입금 전용.

### 발견·수정
- 결제 3페이지(payment/deposit-payment/balance-payment)에 PG사(KG이니시스·토스페이먼츠·NICE페이먼츠) 로고·약관 문구 잔존 → 전부 무통장 기준으로 제거
- **계좌 불일치 버그**: 계약금/일반결제=국민은행 123-456-789012(placeholder)·(주)밸류링크 / 잔금=우리은행 1005-404-483025·호수회계법인 — 서로 다름(가짜계좌로 송금 위험)
- → PO 제공 실계좌로 통일: **하나은행 / 28791092140507 / 파인더월드** (3페이지 전부)
- 잔금 약관 계약주체 호수회계법인→파인더월드(운영사 기준, 입금자명 placeholder의 "(주)파인더월드" 근거)
- 평가보고서 발행기관 '선명회계법인'은 별개라 미변경

### 검증 (라이브)
- valuelink-platform.vercel.app 3개 결제 페이지 직접 fetch → 하나은행/28791092140507/파인더월드 표시 확인, PG/카드/옛계좌 잔재 0건

### 생성/수정 파일
- payment.html, deposit-payment.html, balance-payment.html (frontend/app/valuation/)
...
### 1단계: 전체 여정 스크린샷 분석 (라이브)
- `tests/sim/journey-screenshots.js` 신규: 방법×6단계(방법선택→가이드→포털→자료제출→계산폼→보고서) = 30컷, SIM_BASE로 로컬/라이브 전환
- 방법별 분석 에이전트 5개 병렬 → 계산 로직은 정상, UI/UX·표시·흐름 문제 다수 발견

### 2단계: 발견 문제 일괄 수정 (수정 에이전트 5 병렬 + 직접 마무리)
- **포털 5종**: 자체 임베드 헤더(김철수/삼성전자) → 공용 헤더 fetch 통일 + 데모데이터 파인더월드/선웅규/FINDE-…-{방식} 통일 (relative-portal은 에이전트 중단분 직접 완료)
- **세무 계산식 표시**: `Math.round` 정수 곱셈으로 식 산술 일치 (3,844×3=11,533→11,532). 최종값 로직 불변
- **본질가치**: 평균순이익 표시를 실제 사용값과 일치(toFixed)
- **세무 가이드 시뮬레이터**: 순자산 80% 하한 반영 → 4,622→4,632 본계산 일치
- **상대가치 결과**: 가중치 0 멀티플(PSR/EV-Sales) 카드 숨김 + 비유동성 할인 전/후 항상 표시
- **가이드**: guide-dcf 8편 누락 재번호(1~10) + YouTube 안내 카드화(5종)
- **자료제출 5종**: 버튼 기본 회색→필수충족 시 파랑 전환 + 안내문구

### 검증 (자기검증 금지 — 별도 에이전트 + 직접 이미지 확인)
- fullsim **32/32 PASS** — 계산 로직 불변(상대가치 13,296 등 그대로)
- 포털/제출 삼성전자·김철수·박회계·SAMSU 잔재 **0**, 포털 5종 공용헤더 적용
- 검증 에이전트가 "추가 수정 3건" 보고했으나 **직접 이미지·소스 대조 결과 전부 오인**(상대가치 카드 실제 숨김됨/힌트 회색/회사명 일관) → 반영 불필요 확인
- 직접 확인: relative·dcf 포털 스크린샷에서 공용헤더+파인더월드+카드숨김 육안 검증

### 산출물
- product 18개 수정: guides×5, portals×5, results×3(tax/intrinsic/relative), submissions×5
- `tests/sim/journey-screenshots.js`, `tests/sim/screenshots/journey/manifest.json` (PNG 30컷은 용량상 미커밋, 재현 가능)
- `_WorkLog/2026_06_14__PHASE_5방식_여정_UIUX수정.md`

---

## 2026-06-14 5개 가치평가 방식 풀시뮬레이션 — 실브라우저 다중 시나리오 (reviewer 교차검증) ✅

### 작업 상태: ✅ 완료 — 32/32 PASS, 0 FAIL, 제품 결함 0건

### 배경
- 직전 세션 "무오류 완성"(b82cfa7)은 *수정* 완료·배포했으나 검증은 방식별 수계산 1건 수준. 풀시뮬레이션 단계로 못 넘어가고 끊김.
- `tests/sim/`에 dcf/relative 유저여정형 스크립트만 존재(2/5, 기본값 1회 클릭) — 다중시나리오 아님.

### 수행
- **다중시나리오 실브라우저 하니스** 신규 작성: `tests/sim/fullsim-multiscenario.js`
  - 5개 방식 × (정상/경계/극단/빈값·음수/가드) = 32 시나리오
  - 입력 주입 → 계산 버튼 클릭 → pageerror/console/alert 캡처 → 헤드라인을 **독립 오라클**(공식 재구현)과 대조 → 결과영역 NaN/Infinity 스캔
  - 정적 서버 자동 기동(자식 프로세스), Playwright headless
- **결과: 32/32 PASS, 제품(계산기 HTML) 결함 0건** — 무오류 작업이 다양한 입력에서 견고함 재확인

### reviewer 교차검증(자기검증 금지) → 갭 반영
- 🔴 relative 비유동성 할인 적용값 미검증 → 검증 추가(9,307=13,296×0.7, 클램프 6,648 ✓)
- 🟡 tax 적자 2건이 80% 하한에 막혀 가중평균 경로 미검증 → "적자+하한미발동(97,000)" 추가 → 손실연도 가중평균 반영 검증(작년 수정 버그 회귀 없음)
- 🟡 결과 섹션 숨김/누락 시 거짓통과 → display:none/missing FAIL 처리
- 손검산 3건 독립 일치 확인(dcf 12,979 / tax 24,000 하한 / asset -12,840 음수NAV)

### 산출물
- `tests/sim/fullsim-multiscenario.js` (하니스), `tests/sim/fullsim-multiscenario-report.json` (리포트)
- `_WorkLog/2026_06_14__PHASE_5방식_풀시뮬레이션.md` (PHASE 기록)
- 제품 코드 변경 없음 (결함 0건)
...
### 최종 완료 내용
1. **S5T2** (테스트 커버리지 향상): Completed & Verified ✅
2. **S5M2** (코드 품질 & 가독성): Completed & Verified ✅
3. **ValueLink PPTX 슬라이드쇼**: 생성 완료 ✅
4. **ValueLink HTML 슬라이드쇼**: 생성 완료 ✅

---

## 2026-02-23 19점 로드맵: 3개 Task 추가 (S5F8, S5T2, S5M2) ✅

### 작업 상태: ✅ Task 추가 및 실행 완료
