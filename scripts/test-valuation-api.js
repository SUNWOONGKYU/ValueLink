/**
 * @description 5개 평가 방법별 API End-to-End 테스트
 *
 * 테스트 흐름:
 * 1. Next.js 개발 서버 실행 (http://localhost:3000)
 * 2. POST /api/valuation/execute - 평가 실행
 * 3. 결과 검증
 * 4. HTML 보고서 미리보기 생성
 *
 * 사용법:
 * 1. 터미널 1: npm run dev (Next.js 서버 실행)
 * 2. 터미널 2: node scripts/test-valuation-api.js
 */
...
async function checkServer() {
  try {
    const response = await callAPI('/api/valuation/execute?method=dcf', 'GET', null);
...
async function testMethod(methodKey) {
  ...
  try {
    // Step 5: 평가 실행 API 호출
    log('\n[Step 5] 평가 실행 API 호출 중...', 'blue');
    log(`POST /api/valuation/execute`, 'yellow');
    log(`입력 데이터: ${JSON.stringify(input, null, 2)}`, 'yellow');

    const response = await callAPI('/api/valuation/execute', 'POST', input);

    if (response.statusCode !== 200) {
      throw new Error(`API 호출 실패: ${response.statusCode} - ${JSON.stringify(response.data)}`);
    }
...
// 메인 실행
async function main() {
  log('\n🚀 5개 평가 방법별 API End-to-End 테스트 시작\n', 'cyan');

  // 서버 연결 확인
  log('🔍 Next.js 개발 서버 연결 확인 중...', 'yellow');
  const serverRunning = await checkServer();

  if (!serverRunning) {
    log('\n❌ Next.js 개발 서버에 연결할 수 없습니다.', 'red');
    log('\n다음 명령어로 서버를 먼저 실행해주세요:', 'yellow');
    log('  터미널 1: npm run dev', 'yellow');
    log('  터미널 2: node scripts/test-valuation-api.js', 'yellow');
    log('\n또는 다음 명령어로 한 번에 실행:', 'yellow');
    log('  npm run dev & sleep 5 && node scripts/test-valuation-api.js', 'yellow');
    process.exit(1);
  }
